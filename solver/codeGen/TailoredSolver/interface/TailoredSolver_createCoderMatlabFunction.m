...
% 
% TailoredSolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the TailoredSolver FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% TailoredSolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% TailoredSolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% TailoredSolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function TailoredSolver_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'TailoredSolver_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'TailoredSolver_block';
    end
    ...
    function_name = 'TailoredSolver_coderFunction.m';
    position = [170, 99, 650, 299];
    parameter_sizes = struct('lb', '[480,1]',...
                             'ub', '[400,1]',...
                             'hu', '[200,1]',...
                             'xinit', '[10,1]',...
                             'x0', '[480,1]',...
                             'all_parameters', '[1240,1]',...
                             'num_of_threads', '[1,1]');
    if useCompactBlock
        function_name = 'TailoredSolver_coderFunctioncompact.m';
        position = [170, 99, 650, 299];
        parameter_sizes = struct('lb', '[480,1]',...
                                 'ub', '[400,1]',...
                                 'hu', '[200,1]',...
