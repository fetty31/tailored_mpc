								 ...
								 solver_int32_default threadID   /* Id of caller thread                                */)
{
	...
	TailoredSolver_callback_float w[85];
	
    /* temporary storage for AD tool sparse output */
    TailoredSolver_callback_float this_f = (TailoredSolver_callback_float) 0.0;
    TailoredSolver_callback_float nabla_f_sparse[3];
    
    
    TailoredSolver_callback_float c_sparse[4];
    TailoredSolver_callback_float nabla_c_sparse[15];
	...
	if ((stage >= 79) && (stage < 80))
	{
		out[0] = &this_f;
		out[1] = nabla_f_sparse;
		TailoredSolver_objective_80(in, out, NULL, w, 0);
		if (nabla_f != NULL)
		{
			nrow = TailoredSolver_objective_80_sparsity_out(1)[0];
			ncol = TailoredSolver_objective_80_sparsity_out(1)[1];
			colind = TailoredSolver_objective_80_sparsity_out(1) + 2;
			row = TailoredSolver_objective_80_sparsity_out(1) + 2 + (ncol + 1);
			TailoredSolver_sparse2fullcopy(nrow, ncol, colind, row, nabla_f_sparse, nabla_f);
		}
