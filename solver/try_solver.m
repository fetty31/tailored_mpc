...
N = 20;             %Horizon length
Npar = 25;          %Number of real time params
delta_t = 0.020;    %Time discretization
...
for e=1:its
    ...
    for i=1:N
        ...
        %param = [dRd, m, I,   Lf,    Lr,  Dr,    Df,     Cr,  Cf,   Br,      Bf,      u_r,  g,    Cd,   rho, Ar, q_slip,  q_n, q_mu, q_s, dMtv, t_fact, q_slack_track, vx, k]
        param_k = [1, 240, 93, .708, .822, 3152.3, 2785.4, 1.6, 1.6, 10.1507, 10.8529, .045, 9.81, .8727, 1.255, 1, 1,      10,  1,   100,  1,  delta_t,      0.0,      10, k]';
    ...
    z_lb = [0.0, -100, -300, -deg2rad(23), -3, -deg2rad(50), -3, -deg2rad(40)]';
    z_ub = [100, 300, deg2rad(23), 3, deg2rad(50), 3, deg2rad(40)]';
    
    lb = repmat(z_lb, N, 1);
    lb2 = repmat(z_lb(2:end), N, 1);
    ub = repmat(z_ub, N, 1);
    ...
    if e==1
        problem.x0 = [0.0;x0];
        problem.xinit = [0,0,deg2rad(5), 0.5, 0.05, .01, .01]';
    else
%         problem.x0 = repmat([0,0,controls(5*(e-2)+4),controls(5*(e-2)+5),states(5*(e-2)+1),states(5*(e-2)+2),states(5*(e-2)+3),states(5*(e-2)+4),states(5*(e-2)+5)]',N,1);
%         problem.xinit = [0,0,controls(5*(e-2)+4),controls(5*(e-2)+5),states(5*(e-2)+1),states(5*(e-2)+2),states(5*(e-2)+3),states(5*(e-2)+4),states(5*(e-2)+5)]';
    end
    ...
    controls = [controls;output.U(4*latency+1:4*latency+4)];
    states = [states;output.X(4*latency+1:4*latency+4)];
